...
function read( _sReplaceChar, _tHistory, _tAutoComplete )
    ...
    while true do
        ...
        elseif sEvent == "key" then
			...
			elseif param == keys.tab then
				local sMatch = sLine:gmatch("([^% ]*)$")()
				if _tAutoComplete and sMatch and sMatch:len() > 0 then
					
					for nIndex = 1, #_tAutoComplete do
						if _tAutoComplete[nIndex]:len() > sMatch:len() then
							if string.sub(_tAutoComplete[nIndex], 1, string.len(sMatch)) == sMatch then
								sLine = sLine:sub(1, sLine:len() - sMatch:len()) .. _tAutoComplete[nIndex]
